...
class DashedRect extends StatelessWidget {
  ...
  const DashedRect({this.color = Colors.black, this.strokeWidth = 1.0, this.gap = 5.0});
...
class DashRectPainter extends CustomPainter {
  ...
  Path getDashedPath({
    ...
    while (currentPoint.x <= b.x && currentPoint.y <= b.y) {
      double x = double.parse(currentPoint.x.toString());
      double y = double.parse(currentPoint.y.toString());
      shouldDraw ? path.lineTo(x, y) : path.moveTo(x, y);
